...
void computePositionOffsets(out float xOffset, out float yOffset)
{
   import std.math;

   const loopDuration = 5.0;
   const scale = 2 * PI / loopDuration;

   auto elapsedTime = SDL_GetTicks() / 1000.0;

   auto currTimeThroughLoop = fmod(elapsedTime, loopDuration);
...
{
   auto newData = vertexData.dup;

   for (auto i = 0; i < vertexData.length / 2; i += 4)
   {
      newData[i] += xOffset;
      newData[i+1] += yOffset;
   }
   ...
   dataBufferObject.bind();
   glBufferSubData(GL_ARRAY_BUFFER, 0, vertexData.sizeof / 2, newData.ptr);
   dataBufferObject.unbind();
}
...
void main()
{
   ...
   // Window, please
   auto window = Window("CPU Animation",
